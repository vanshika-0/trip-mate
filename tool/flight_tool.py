###live flights ki information dene wala tool - from loc to loc 
...
DEFAULT_ORIGIN_IATA = os.getenv(
    "DEFAULT_ORIGIN_IATA",
    os.getenv("DEFAULT_ORIGIN_DATA", "DAC"),
)
...
def parse_route(query: str):
    ...
    # Pattern: to Y from X
    match = re.search(
        r"\bto\s+(?!go\b)(.+?)\s+\bfrom\s+(.+?)(?:\s+(?:on|for|under|including|with|in|at)\b|[.!?]|$)",
        q_lower,
    )
    ...
    if len(mentions) == 1:
        ...
        default_origin = resolve_location_to_iata(DEFAULT_ORIGIN_IATA) or "DAC"
...
def search_flights(query: str, limit: int = 10):
    ...
    try:
        ...
        # Do not return the exception text: requests may include the full URL,
        # including the private access_key query parameter.
        return f"Flight API request failed ({type(e).__name__}). Check your network connection."
    except ValueError:
